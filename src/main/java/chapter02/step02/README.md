...
하지만 그래도 이 설계가 정말 잘된 설계이고 객체지향적인 코드라고 할 수 있다.

여기서 이야기하고자 하는 건 구현과 관련된 모든 것들이 트레이드 오프 대상이 될 수 있다는
것이다. 우리들이 작성하는 모든 코드에는 합당한 이유가 필요하고 아주 사소한 결정이라도
트레이드 오프를 통해 얻어진 결론과 아닌 것의 결과의 차이는 크다.

고민하고 트레이드 오프를 하자.

### 코드 재사용

상속은 코드를 재사용하기 위해 널리 사용되는 방법이다. 하지만 객체지향 설계에서는 상속보다는
합성이 더 좋은 방법이다. 합성은 다른 객체의 인스턴스를 자신의 인스턴스 변수로 포함해서
재사용하는 방법을 말한다.
...
상속은 코드 재사용을 위해 널리 사용되지만 캡슐화를 위반 하고 설계를 유연 하게 하지
못한 다는 측면에서 안좋은 영향을 미친다.

상속을 이용하기 위해 자식 클래스는 부모 클래스를 알고 있어야하고 그렇기 때문에 캡슐화를
위반하게 된다.

상속을 과도하게 한다면 결과적으로 부모 클래스를 바꿀 때마다 자식 클래스에도 변경이 발생
하는 아주 안좋은 상황에 처한다.

두 번째는 설계가 유연하지 않다는 것이다. 실행 시점에 객체의 종류를 변경하는 것이
불가능한 상속 특성상 실행 시점에 금액 할인 정책인 영화를 비율 할인 정책으로 변경한다고
가정했을 때 인스턴스를 변경 해야 한다. 이 때 생성된 객체의 클래스를 변경하는 기능을
지원하지 않기에 이 문제를 해결할 수 있는 방법은 새로운 인스턴스를 생성 후
AmountDiscountMovie의 상태를 복사하는 것 뿐이다.

이것은 부모 클래스와 자식 클래스가 붙어 있기 때문에 발생하는 문제다.

다음과 같이 Movie 클래스에 메서드를 추가해보자.
...
이 예제를 통해 상속보다 인스턴스 변수로 관계를 연결한 원래의 설계가 더 유연하다는 사실
을 알 수 있다. Movie가 DiscountPolicy를 포함하는 이 방법 역시 코드를 재사용하는 것이
핵심이다.

### 합성

Movie 요금을 계산하기 위해 DiscountPolicy의 코드를 재사용한다. 이 방법이 상속과 다른
점은 상속이 부모 클래스의 코드와 자식 클래스의 코드를 컴파일 시점에 하나의 단위로 강하게
결합하는데 비해 Movice가 DiscountPolicy 인터페이스를 통해 약하게 결합된단는 것이다.

실제로 Movie는 DiscountPolicy가 외부에 메서드를 제공한다는 사실만 알고 내부 구현에
대해서는 전혀 알지 못한다. 이처럼 인터페이스에 정의된 메시지를 통해서만 코드를
재사용하는 방법을 합성이라고한다.

> 상속보다는 합성을 사용하여 코드 재사용을 하자